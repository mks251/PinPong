...
public class PingPongGameEngine implements Runnable,MouseMotionListener, KeyListener, GameConstants{
	...
	private int ballX;  // координатаX мяча!
	private int ballY;  // координатаY мяча!
	...
	// Обязательные методы из интерфейсаMouseMotionListener 
	...
	// Обязательные методы из интерфейсаKeyListener
	...
	// Шаг3. Перемещать ракетку компьютера вверх или вниз,
	...
	// Шаг5. Обновить счет, если мячв зеленой области, но не движется
	if (isBallOnTheTable()){
	if (ballX > BALL_MAX_X ){
	computerScore++;
	displayScore();
